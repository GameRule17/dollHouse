...
public final class DataBaseLoader {
    ...
    public static void initializeDBConnection () {
        try {
            conn = DriverManager.getConnection("jdbc:h2:./database/statistics;DB_CLOSE_ON_EXIT=FALSE;AUTO_SERVER=TRUE");
    ...
    public static void createTable () {
        try {
            ...
            stmt.executeUpdate("CREATE TABLE IF NOT EXISTS statistics (id INT PRIMARY KEY AUTO_INCREMENT, username VARCHAR(255), points INT, time TIME)");
            stmt.close(); 
        } catch (SQLException ex) {
            System.out.println("Errore: " + ex.getMessage());
        }
    ...
    public static String printUserData (String mode) {
        ...
        try {
            ...
            while (rs.next()) {
                result += "<tr> <td>" +  rs.getString("username") + "</td> <td>" + rs.getInt("points") + "</td><td>" + rs.getTime("time") + "</td> </tr>";
            }
            stmt.close();
        } catch (SQLException ex) {
            System.out.println("Errore: " + ex.getMessage());
